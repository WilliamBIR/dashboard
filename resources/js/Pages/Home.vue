        ...
        <div class="py-12">
            <div class="max-w-full mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <home :sucursales="sucursales" :rol="user.role"/>
                </div>
            </div>
        </div>
    </app-layout>
...
<script>
    ...
    import Home from '@/Jetstream/Home.vue'
    ...
    export default defineComponent({
        ...
        created(){
            if(!this.$inertia.path){
                this.$inertia.path = 'http://192.168.100.123:2545';
                sessionStorage.setItem('path', this.$inertia.path);
                this.$inertia.connect = ()=>{
                    axios.post(this.$inertia.path+'/authAdmin',
                        {id:this.user.id,password:this.user.password}
                    ).then((res)=>{
